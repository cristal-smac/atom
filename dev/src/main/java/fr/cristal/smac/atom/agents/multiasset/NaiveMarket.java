 ...
 *
 */
public class NaiveMarket extends ModerateAgent
{

    int period;
    long initPrice;

    public NaiveMarket(String name, long cash, long initPrice, int period)
    {
        super(name, cash);
        this.period = period;
    ...
    public Order decide(String invest, Day day)
    {
        ...
        if (day.currentPeriod().currentTick() % period == 1) // on rééquilibre
        {
            ...
            {
                if (ob.bid.size()>0)
                o = new MarketOrder(invest, myId + "", LimitOrder.ASK, diff);
            }
...
*/
                ...
                // ATTNTION : ce n'est pas parce qu'il y a des ASK, qu'il y a la bonne quantité dispo
                if (ob.ask.size() > 0 && (Math.abs(diff) * ob.ask.first().price) < cash/n)
                    o = new MarketOrder(invest, myId + "", LimitOrder.BID, Math.abs(diff));
